...
export default function SettingsPage() {
    ...
    supabase.from("profiles").select("settings").eq("id", currentUser.id).single()
      .then(({ data }) => {
        const s = data?.settings || {
          darkMode: false, emailAlerts: true, multiUser: true, roleAccess: false, securityLock: true,
        };
        // Sync ThemeContext with saved setting on load
        s.darkMode = isDark;
        setSettings(s);
      });
  }, [currentUser]);
  ...
  const toggleSetting = async (key) => {
    ...
    // If toggling dark mode, update ThemeContext (which persists to localStorage)
    ...
    if (error) { console.error("Failed to save settings:", error); alert("Failed to save settings"); }
    setSaving(false);
  };
