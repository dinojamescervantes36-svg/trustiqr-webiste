...
export default function ManageTemplates() {
  ...
  const loadTemplates = async () => {
    ...
    if (data) setTemplates(data.filter(f => f.name !== ".emptyFolderPlaceholder").map(f => ({ id: f.id || f.name, name: f.name.replace(/^\d+_/, ""), fullPath: `templates/${currentUser.id}/${f.name}`, type: f.name.toLowerCase().endsWith(".pdf") ? "PDF" : "IMAGE", createdAt: f.created_at?.slice(0, 10) || "—" })));
  };
  ...
  const handleFileSelected = async (e) => {
    ...
    if (error) alert("Upload failed: " + error.message); else await loadTemplates();
  ...
  const handleView = async (template) => {
    ...
    if (data?.signedUrl) window.open(data.signedUrl, "_blank"); else alert("Could not open file.");
  };

  const handleDelete = async (template) => {
    if (!confirm("Delete this template permanently?")) return;
    const { error } = await supabase.storage.from("certificates").remove([template.fullPath]);
    if (!error) setTemplates(prev => prev.filter(t => t.id !== template.id)); else alert("Delete failed: " + error.message);
  };

  const handleUseTemplate = (template) => {
    const query = new URLSearchParams({ templateId: template.id, templateName: encodeURIComponent(template.name) }).toString();
        ...
        <div className="template-grid">
          {templates.length === 0 && (
            <div className="card" style={{ gridColumn: "1/-1", textAlign: "center", padding: 48 }}>
              <FiFile size={36} style={{ marginBottom: 12, opacity: 0.3, color: "var(--text-faint)" }} />
          ...
          {templates.map(template => (
            <div key={template.id} className="card template-card">
              ...
              <div className="template-actions">
                <button onClick={() => handleUseTemplate(template)} style={{ background: "var(--bg-hover)", color: "var(--primary)" }}>Use</button>
